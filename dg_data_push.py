...
def push_file():
    with open(FILENAME, 'r') as ingest_file:
        datagen, headers = multipart_encode({"Authorization": "bearer" + access_token, "cycle-data": ingest_file, "routing-key": "data.ingestion.dg"})
...
while True:
    ...
    h = randint(0, 100)
    l = randint(0, 100)
    output.append(str(ts) + ',' + TAG + ',' + str(m) + ',' + str(h) + ',' + str(l))
